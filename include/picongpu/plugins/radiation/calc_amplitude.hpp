...
struct Retarded_time_1
{
    ...
    HDINLINE picongpu::float_32 operator()(const picongpu::float_32 t,
                                const vector_32& n, const Particle & particle) const
    {
        const vector_32 r(particle.get_location<When::now > ()); // location
	//accumulate into a double precision buffer, then return truncated
	float_64 accumulator = n*r/ picongpu::SPEED_OF_LIGHT;
	accumulator -= t;
        //return (picongpu::float_32) (t - (n * r) / (picongpu::SPEED_OF_LIGHT));
	return static_cast<picongpu::float_32>(-1*accumulator);
    }
...
struct Old_Method
{
    ...
    HDINLINE vector_32 operator()(const vector_32& n, const Particle& particle, const picongpu::float_32 delta_t) const
    {
        const vector_32 beta(particle.get_beta<When::now > ()); // beta = v/c
	//Temporary change to 64 bit precision
